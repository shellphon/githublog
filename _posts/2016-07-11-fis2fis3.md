...
category: Hello Code
---
刚接触fis2的时候，就已经出fis3了，不过项目都是用fis2做的，也不熟悉，也就没想过迁移的事。
趁着空闲，刷了下fis2的源码，再学习一下fis3，开始针对着项目打算做一下fis3的迁移尝试。
...
先从简单入手，即项目本地开发模式下的配置，在我理解下，项目主要可以分为四类产出模式：  
> 本地开发模式：dev  
  打包不压缩模式: 打包部分设定合并的资源  
...
1. 资源同名依赖： fis2默认解析模板和js的时候会做js和css的同名依赖，fis3则需要主动配置`useSameNameRequire: true`
2. 模板ftl的解析资源依赖：需要用到插件`fis3-preprocessor-extlang`,原本插件并没有支持ftl文件，但是，直接把smarty的模板解析部分代码复制并把对应的tpl改成ftl，并加上自定义标签的起始标识符即可，关于这个插件，后面再做一下描述
...
~~~
【这个moduleId是个坑】原本以为配置widget下的js匹配moduleId设置就万事大吉了，然而，发现，hook-commonjs对js的处理中，当js内有require其他js的情况下，也是会出现require不带后缀的情况，看了下源码，hook-commonjs中对lookup事件时触发的处理，lookup.js有处代码如下：
~~~
// 跨模块引用 js
...
~~~
原本只要这个方法返回false就可以规避js资源moduleId的干涉了，然而这里并不是；  
...
为了迁移更快（其实是懒），我将fis2的pack对象数据直接独立成一个set-pack.json文件（缺点就是不能加注释了）  
...
~~~
现在想到的还有一个缺点就是，fis.match可以写成链式调用的 ，但是这里就被我断开了。  
